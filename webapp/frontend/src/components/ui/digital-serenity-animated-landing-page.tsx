  ...
  useEffect(() => {
    const animateWords = () => {
      ...
      wordElements.forEach((word) => {
        const rawDelay = word.getAttribute("data-delay") ?? "0";
        const delay = Number.parseInt(rawDelay, 10) || 0;
        window.setTimeout(() => {
    ...
    const handleScroll = () => {
      if (!scrolled) {
        ...
        floatingElementsRef.current.forEach((el, index) => {
          const baseDelaySeconds = Number.parseFloat(el.style.animationDelay || "0");
          window.setTimeout(() => {
            el.style.animationPlayState = "running";
            el.style.opacity = "";
          }, baseDelaySeconds * 1000 + index * 100);
        });
      }
    };
        ...
        <div className="relative z-10 flex min-h-screen flex-col items-center justify-between px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20">
          <div className="text-center">
            <h2 className="font-mono text-xs font-light uppercase tracking-[0.2em] text-slate-300 opacity-80 sm:text-sm">
              <span className="word-animate" data-delay="0">Droit gabonais.</span>
              <span className="word-animate" data-delay="300">Clarté immédiate.</span>
            </h2>
          ...
          <div className="relative mx-auto max-w-5xl text-center">
            <h1 className="text-decoration-animate text-3xl leading-tight tracking-tight text-slate-50 sm:text-4xl md:text-5xl lg:text-6xl">
              <div className="mb-4 md:mb-6">
                <span className="word-animate" data-delay="700">Comprendre</span>
                <span className="word-animate" data-delay="850">vos</span>
                <span className="word-animate" data-delay="1000">droits,</span>
              </div>
              <div className="text-xl font-thin leading-relaxed tracking-wide text-slate-300 sm:text-2xl md:text-3xl lg:text-4xl">
                <span className="word-animate" data-delay="1400">avec</span>
                <span className="word-animate" data-delay="1550">des</span>
                <span className="word-animate" data-delay="1700">sources</span>
                <span className="word-animate" data-delay="1850">fiables</span>
                <span className="word-animate" data-delay="2000">et</span>
                <span className="word-animate" data-delay="2150">un</span>
                <span className="word-animate" data-delay="2300">langage</span>
                <span className="word-animate" data-delay="2450">simple</span>
                <span className="word-animate" data-delay="2600">pour</span>
                <span className="word-animate" data-delay="2750">agir.</span>
              </div>
            </h1>
            <div className="absolute -left-6 top-1/2 h-px w-3 -translate-y-1/2 transform bg-slate-300 opacity-0 sm:-left-8 sm:w-4" style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "3.2s" }} />
            <div className="absolute -right-6 top-1/2 h-px w-3 -translate-y-1/2 transform bg-slate-300 opacity-0 sm:-right-8 sm:w-4" style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "3.4s" }} />
          </div>

          <div className="text-center">
            <div className="mx-auto mb-4 h-px w-12 bg-gradient-to-r from-transparent via-slate-300 to-transparent opacity-30 sm:w-16" />
            <h2 className="font-mono text-xs font-light uppercase tracking-[0.2em] text-slate-300 opacity-80 sm:text-sm">
              <span className="word-animate" data-delay="3000">Questionnez.</span>
              <span className="word-animate" data-delay="3200">Vérifiez.</span>
              <span className="word-animate" data-delay="3400">Décidez.</span>
            </h2>
            <div className="mt-6 flex justify-center gap-3 opacity-0" style={{ animation: "word-appear 1s ease-out forwards", animationDelay: "4.2s" }}>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <Link href="/chatbot" className="rounded-full border border-slate-300/40 px-5 py-2 text-sm text-slate-100 transition hover:bg-slate-100/10">
                Ouvrir le chatbot
              </Link>
              <Link href="#contact" className="rounded-full border border-slate-400/20 px-5 py-2 text-sm text-slate-300 transition hover:bg-slate-100/5">
                Nous contacter
              </Link>
            </div>
          </div>
